...
def run_india_2015_2025_refined_audit():
    print("=== ACE: Arion Crash Engine - INDIA (NIFTY) 2015-2025 REFINED AUDIT ===")
    ...
    def scan_period(start_year, end_year):
        ...
        for date in weeks:
            ...
            # Use real data from havoc_logic
            ...
            if model_05.predict_proba(X)[:, 1][0] >= 0.75: results['-5%'] += 1 # Slightly lower threshold for pulses to catch them better
    ...
    print("="*60)
    print(f"TIER             | Signals detected")
    print(f"----------------------------------------------------------")
    print(f"Structural (-20%)| {past['-20%']} Signals")
    print(f"Correction (-10%)| {past['-10%']} Signals")
    print(f"Micro-Pulse (-5%)| {past['-5%']} Signals")
    print("="*60)
    print("NOTE: COVID-2020 Crash and recent volatility accounted for.")

if __name__ == "__main__":
    run_india_2015_2025_refined_audit()

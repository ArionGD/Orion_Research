...
def run_india_2015_2025_audit_debug():
    print("=== ACE: Arion Crash Engine - INDIA (NIFTY) 2015-2025 DEBUG AUDIT ===")
    ...
    def scan_period(start_year, end_year):
        ...
        for date in weeks:
            ...
            X = df_row[features]
            p20 = model_20.predict_proba(X)[:, 1][0]
            p10 = model_10.predict_proba(X)[:, 1][0]
            p05 = model_05.predict_proba(X)[:, 1][0]
            
            max_probs['-20%'] = max(max_probs['-20%'], p20)
            max_probs['-10%'] = max(max_probs['-10%'], p10)
            max_probs['-5%'] = max(max_probs['-5%'], p05)
            
            if p20 >= 0.80: results['-20%'] += 1
            if p10 >= 0.80: results['-10%'] += 1
            if p05 >= 0.80: results['-5%'] += 1
            
        return results, max_probs

    print("\nScanning INDIA Period: 2015 - 2025...")
    results, max_probs = scan_period(2015, 2025)
    
    print("\n" + "="*60)
    print("ACE: INDIA (NIFTY) 2015-2025 MULTI-TIER AUDIT")
    print("="*60)
    print(f"TIER             | Signals | Max Prob Found")
    print(f"----------------------------------------------------------")
    print(f"Structural (-20%)| {results['-20%']}       | {max_probs['-20%']:.1%}")
    print(f"Correction (-10%)| {results['-10%']}       | {max_probs['-10%']:.1%}")
    print(f"Micro-Pulse (-5%)| {results['-5%']}       | {max_probs['-5%']:.1%}")
    print("="*60)

if __name__ == "__main__":
    run_india_2015_2025_audit_debug()
